class Image < ApplicationRecord
  ...
  def self.value_and_update(new_value_data)
    # { user_id: current_user.id, image_id: params[:image_id].to_i, value: value }
    image_id = new_value_data[:image_id]
    #theme_id = new_value_data[:theme_id]
    puts "in value_and_update: image_id = #{image_id} \n"
    Value.create(new_value_data) # Создается новая запись в базе
    ave_value = Value.calc_average_value(image_id).round # Высчитывается среднее значение оценок по конкретной картинке
    puts "in value_and_update: ave_value = #{ave_value} \n"
    update_ave_value(image_id, ave_value) # Обновление средней оценки по каждой картинке в таблице images
